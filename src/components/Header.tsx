...
export function Header() {
  ...
  const recalculateNav = useCallback(() => {
    ...
    for (let i = 0; i < mainMenus.length; i++) {
        ...
        const widthWithMoreMenu = cumulativeWidth + moreMenuWidth + itemGap + safetyBuffer;
        ...
        if (widthWithMoreMenu > navContainerWidth) {
            ...
            if (finalVisibleWidth + moreMenuWidth + itemGap + safetyBuffer > navContainerWidth && newVisible.length > 0) {
  ...
  const renderDesktopNav = () => (
    <nav ref={navRef} className="hidden md:flex flex-1 items-center justify-center gap-3 font-medium min-w-0">
      ...
      <div className="absolute top-0 left-0 -z-50 opacity-0 pointer-events-none flex">
        {mainMenus.map((menu) => (
          <Link
            key={menu.id}
            href={menu.link ?? "#"}
            ref={el => { if (el) itemWidths.current.set(menu.id, el.offsetWidth); }}
            className={cn(
        ...
        {/* Đo cả nút "Thêm" */}
        <div ref={moreMenuRef} className="flex items-center gap-1 hover:text-primary">
          {/* SỬA: Dùng text-sm (14px) làm cơ sở, lg:text-sub2 (18px) cho desktop */}
