...
export default function AccountNavFab() {
  ...
  return (
    <div className="fixed bottom-10 left-10 z-50 flex flex-col-reverse items-start gap-4">
      ...
      <button
        onClick={toggleMenu}
        className={`flex h-12 w-12 items-center justify-center rounded-full text-white shadow-lg transition-all duration-300 hover:bg-opacity-90 ${isOpen ? "bg-red-500 rotate-45" : "bg-blue-600" // Giả sử primary là blue-600
          }`}
      ...
      <div className={`flex flex-col gap-3 transition-all duration-300 ${isOpen ? "translate-y-0 opacity-100" : "translate-y-10 opacity-0 pointer-events-none"
        }`}
      >
        ...
        {menuItems.map((item, index) => {
          ...
          return (
            <Link
              ...
              className="group flex items-center gap-3"
            >
              <div className={`flex h-10 w-10 items-center justify-center rounded-full text-white shadow-md transition-transform hover:scale-110 ${isActive ? "ring-2 ring-offset-2 ring-blue-600" : ""
                } ${item.color}`}>
